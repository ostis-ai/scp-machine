...
SCPOperatorSearchSetStr5::SCPOperatorSearchSetStr5(ScAgentContext & ctx, ScAddr addr)
...
sc_result SCPOperatorSearchSetStr5::Execute()
{
  ...
  case 0x00001:
  {
    ...
    while (iter->Next())
    {
      ScAddr elem0 = m_memoryCtx.GetArcSourceElement(iter->Get(2));
      ScAddr elem2 = m_memoryCtx.GetArcTargetElement(iter->Get(2));
