...
SCPOperatorStringReplace::SCPOperatorStringReplace(const std::unique_ptr<ScMemoryContext> &ctx, ScAddr addr): SCPOperatorElStr5(ctx, addr)
{
}
...
sc_result SCPOperatorStringReplace::Execute()
{
    ...
    if (!operands[3]->GetValue().IsValid())
    {
        ...
        string str3 = Utils::scLinkGetString(ms_context, operands[3]->GetValue());
